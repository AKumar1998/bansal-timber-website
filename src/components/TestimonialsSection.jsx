...
export default function TestimonialsSection() {
  ...
  // Fetch testimonials
  useEffect(() => {
    fetch("/api/testimonials/get_testimonials.php")
      .then((res) => res.json())
      .then((data) => {
      ...
      <div className="relative mt-16 flex justify-center items-center overflow-hidden">
        ...
        >
          {testimonials.map((t, i) => (
            ...
            >
              <div className="relative rounded-2xl overflow-hidden shadow-lg h-[450px]"> {/* 🔸 height increased */}
                <img
                  src={t.image}
                  alt={t.name}
                  className="absolute inset-0 w-full h-full object-cover"
                />
                <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center text-center text-white p-8">
                  <p className="text-xl italic mb-6 max-w-2xl mx-auto leading-relaxed">{t.testimonial_text}</p>
                  <h3 className="text-2xl font-semibold">{t.name}</h3>
                  <p className="text-sm text-gray-300 mt-2">{t.project_name}</p>
                </div>
              </div>
            </div>
          ))}
